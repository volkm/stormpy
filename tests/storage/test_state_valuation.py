...
class TestStateValuationTransformer:
    def test_transform_die(self):
        ...
        svt = stormpy.storage.StateValuationTransformer(model.state_valuations)
        manager = program.expression_manager
        svt.add_boolean_expression(
            manager.create_boolean_variable("s_exceeds_three"),
            stormpy.storage.Expression.Geq(manager.get_variable("s").get_expression(), manager.create_integer(4)),
        )
        svt.add_integer_expression(
            manager.create_integer_variable("sum"),
            stormpy.storage.Expression.Plus(manager.get_variable("d").get_expression(), manager.get_variable("s").get_expression()),
        )
