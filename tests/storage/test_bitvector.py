...
class TestBitvector:
    def test_init_default(self):
        bit = stormpy.storage.BitVector()
        assert bit.size() == 0
        assert bit.number_of_set_bits() == 0

    def test_init_length(self):
        bit = stormpy.storage.BitVector(10)
        assert bit.size() == 10
        assert bit.number_of_set_bits() == 0
        bit = stormpy.storage.BitVector(5, True)
        assert bit.size() == 5
        assert bit.number_of_set_bits() == 5

    def test_init_vector(self):
        bit = stormpy.storage.BitVector(5, [2, 3])
        assert bit.size() == 5
    ...
    def test_init_bitvector(self):
        bit = stormpy.storage.BitVector(7, [0, 6])
        bit2 = stormpy.storage.BitVector(bit)
        assert bit == bit2
