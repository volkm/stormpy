...
void define_sparse_model(py::module& m, std::string const& vtSuffix) {
    ...
    model.def_property_readonly("supports_uncertainty", &SparseModel<ValueType>::supportsUncertainty, "Flag whether model supports uncertainty via intervals")
        ...
        .def_property_readonly("transition_matrix", &getTransitionMatrix<ValueType>, py::return_value_policy::reference, py::keep_alive<1, 0>(),
                               "Transition matrix")
        .def_property_readonly("backward_transition_matrix", &SparseModel<ValueType>::getBackwardTransitions, py::return_value_policy::reference,
                               py::keep_alive<1, 0>(), "Backward transition matrix")
        .def(
            "get_reward_model",
